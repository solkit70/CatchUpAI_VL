...
**Topic 설명**:
```
워싱턴주 요양보호사 3개 경로(HCA·IP·NAC)를 조사해 하는 일·자격 취득 절차·급여와 복지·
거주지에서 가능한 일자리를 파악하고, 본인에게 맞는 역할에 실제로 지원해 교육을 이수하고
일할 수 있는 상태까지 만든다. 조사 결과는 한인 커뮤니티용 안내 자료와 영상으로 공유한다.

**알고 있는 3개 경로** (명칭·정의는 M1에서 1차 출처로 검증 대상):
...
**예상 학습 기간**: `4~6주 (여유 있게, 다른 Topic·라이브 방송과 병행)`
...
- web-data-extraction 방법론 — JS 렌더링 페이지의 데이터 원천 직접 조회
- Remotion (영상 산출물)
- 전화·이메일 기관 문의 — **사용자가 직접 수행**, AI가 대리하지 않는다
```

**사전 지식**:
```
필수:
- 없음. **의료·돌봄 분야 무경력**을 전제로 한다

권장:
- 영어 문서 읽기 — WA DSHS·DOH 공식 문서가 전부 영문
- Datacenter-Workforce-Programs Topic에서 익힌 조사 규율
...
- [ ] Bonney Lake / Tehaleh 인근에서 실제 지원 가능한 고용주·기관 목록을 확보한다
...
**참조 자료**:
```
**공식 기관 (URL은 M1 실습 중 확보)**
- WA DSHS (Department of Social and Health Services) — 장기요양·IP
- WA DOH (Department of Health) — HCA·NAC 자격 발급
- Consumer Direct Care Network Washington — IP 관련
- SEIU 775 Benefits Group — 교육·복지

> ⚠️ 위 기관명 일부는 **사용자 사전 지식과 일반 통념에 기반한 추정**이다.
> 실제 담당 기관과 명칭은 M1에서 1차 출처로 확인한다. **확인 전에는 사실로 쓰지 않는다.**

**관련 Topic**
- `Datacenter-Workforce-Programs` — 같은 "조사 → 본인 지원 → 공유" 구조.
  핵심 교훈: **"프로그램이 있다"와 "지금 지원할 수 있다"는 다른 말이다.**
  조사 단계에서 좋아 보이던 경로가 실행 단계에서 닫혀 있는 사례를 겪었다.
```

**vl_materials/ 폴더**:
```
`vl_materials/` 에 아래를 모은다.
- 기관 공식 안내문·PDF 다운로드분
- 교육기관 커리큘럼·일정표
- 영상 제작용 스크린샷

**저장 금지**: 실제 신원 정보(SSN·면허번호·생년월일). 지원서 사본은 해당 필드를 마스킹해 보관한다.
```
...
사용자가 입력한 학습 기간 `4~6주 (여유 있게, 다른 Topic·라이브 방송과 병행)`이 해당 Topic에 적절한지 분석하고 피드백을 제공하세요.
...
**사용자 입력 기간**: 4~6주 (여유 있게, 다른 Topic·라이브 방송과 병행)
**Topic 복잡도**: [간단/중간/복잡]
**권장 기간**: [X주 또는 Y일]
