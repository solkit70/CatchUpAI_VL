...
**Topic 폴더 경로**:
```
C:\AI_study\2026\Changsoo_Vault\Ingest\CatchUpAI_VL\Topics\Chrome-Remote-Desktop\
```

**Roadmap 파일 경로**:
```
vl_roadmap/20260920_RoadMap_Chrome-Remote-Desktop.md
```
...
**사용 가능한 시간**:
```
[예: 1.5시간]   ← 이 Topic 은 1회 1~2시간 단위로 설계돼 있다
```

**오늘의 상태/제약사항** (있다면):
```
[예: 오늘 외출 예정 — M4·M5 를 할 수 있다]
[예: 집에만 있음 — M4·M5 는 못 하니 M1·M3·M6 쪽으로]
[없음]
```

---

### 🔴 이 Topic 의 고정 제약 (매 세션 적용)

1. **독자는 IT 지식이 없는 일반인**이다. 용어를 쓰면 그 자리에서 풀어 쓴다
   (호스트 → 집에 켜 둘 컴퓨터 · 클라이언트 → 밖에서 쓰는 기기 · 세션 → 접속해 있는 동안).
   특정 집단을 한정하는 표현(「시민단체 담당자라면」)은 쓰지 않는다.
2. **화면 캡처의 개인 정보는 반드시 마스킹** — 계정 이메일 · PIN 숫자 · 기기 고유 이름 · 집 주소.
   이 레포는 GitHub 에 공개된다.
3. **보안 항목은 「왜」를 한 줄 먼저** 쓰고 설정 방법을 쓴다.
4. **M4·M5 는 외출 기회가 있어야 한다.** 집에 있는 날이면 다른 모듈을 먼저 한다 —
   순서를 바꿔도 되고, 기회가 오면 그날 M4/M5 로 전환한다.
5. **모듈마다 `troubleshooting/` 문서**를 둔다 (안 될 때 어디를 보는지).
6. **DoD 의 「따라 하기 검증」**을 형식적으로 체크하지 않는다 — 그 문서만 보고 처음 하는
   사람이 끝까지 갈 수 있는지 실제로 짚어 본다.
