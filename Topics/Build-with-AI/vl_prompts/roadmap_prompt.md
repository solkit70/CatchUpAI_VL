...
**Topic 설명**:
```
송재희님의 Build with AI 12부작 자료를 학습하고, Catch Up AI / Builders Lounge 관점에서 "AI로 데모는 만들었는데 왜 서비스는 안 되는가"라는 영상 산출물로 전환한다. 핵심 연구 축은 비개발자 빌더, 문제 구조화, 데이터/워크플로 병목, 에이전트/바이브 코딩, 데모에서 프로덕션으로 넘어가는 기준이다.
```

**학습 목적**:
```
1. Build with AI 12부작의 전체 구조와 핵심 메시지를 Source Map으로 정리한다.
2. Builders Lounge Product Discovery 및 Bila AI Agent 프로젝트와 연결되는 영상 논지를 도출한다.
3. 첫 영상의 타깃 시청자, 후킹 질문, 스토리라인, 시각 자료 후보를 정리한다.
4. 후속 영상 제작을 위한 브리프와 스크립트 초안을 만든다.
```

**예상 학습 기간**: `1주 또는 4-6시간 집중 연구`

---

### 환경 및 사전 지식

**운영 체제**: `Windows 11 / Obsidian Vault`

**주요 도구 및 기술 스택**:
```
Codex, Obsidian, VibeLearn AI, Builders Lounge notes, CatchUpAI_VL topic system, Markdown, 영상 기획/스크립트 작성 도구
```

**사전 지식**:
```
필수:
Builders Lounge의 목적, Catch Up AI 영상 톤, VibeLearn AI Topic/WorkLog 구조

권장:
Build with AI 12부작 자료, Bila AI Agent 프로젝트 계획, Product Discovery 관점, AI agent / vibe coding / production readiness 기본 개념
```
...
**학습 목표** (달성하고 싶은 것):
```
Source Map 1개, 영상 Angle 1개, 영상 Brief 1개, 첫 WorkLog 1개를 만들고 다음 단계에서 스크립트/슬라이드 플랜으로 이어갈 수 있게 한다.
```
