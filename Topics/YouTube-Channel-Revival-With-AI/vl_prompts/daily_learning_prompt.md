...
> ⚠️ **아래 값은 이 Topic 기준으로 채워져 있다.** 세션마다 바뀌는 것은
> 「현재 진행 중인 모듈」·「가장 최근 WorkLog」·「사용 가능한 시간」 셋뿐이다.
...
**Topic 폴더 경로**:
```
C:/AI_study/2026/Changsoo_Vault/Ingest/CatchUpAI_VL/Topics/YouTube-Channel-Revival-With-AI/
```

**Roadmap 파일 경로**:
```
vl_roadmap/20260910_RoadMap_YouTube-Channel-Revival-With-AI.md
```
...
**현재 진행 중인 모듈**:
```
[세션마다 갱신 — 예: M1 - 데이터 파이프라인과 P5 정의]
```

**가장 최근 WorkLog 파일**:
```
[세션마다 갱신 — 예: vl_worklog/20260910_M1_YouTube-Channel-Revival-With-AI.md]
또는
[없음 - 첫 학습 세션]
```
...
**사용 가능한 시간**:
```
[세션마다 갱신 — 로드맵 기준: M1 5h · M2 3h · M3 2.5h · M4 1.5h · M5 2.5h · M6 2.5h]
```

**오늘의 상태/제약사항** (있다면):
```
🔴 발표일이 2026-09-16 (수) 19:00 로 고정돼 있다. 남은 날짜를 매 세션 계산해서 알려 줄 것.
🔴 시간이 모자라면 로드맵의 「⚠️ 시간이 모자랄 때의 우선순위」를 따른다.
   각 모듈 DoD 의 🟥 표시가 최소 완료선이다 — 거기까지만 하고 다음으로 넘어가도 된다.
```
...
**① 기억과 데이터가 어긋나면 즉시 기록한다.**
브레인덤프의 숫자는 사용자의 기억이다. 이미 두 곳에서 어긋남이 확인됐다
(횡보 시작 구독자 수 · 되살리기 시작 시점). 발표에서 숫자를 말하는 이상
**원문의 기억은 전부 데이터로 대조한 뒤에 쓴다.** 어긋난 지점은 WorkLog 에 바로 적는다.

**② 지표를 읽을 때의 한계를 매번 함께 적는다.**
영상별 구독자 증가는 「그 영상의 시청 페이지에서 구독을 누른 횟수」뿐이다.
**기여의 하한선**으로만 읽고, 구독자 감소는 원인 추적에 쓰지 않는다.

**③ 인증 파일은 절대 공개 폴더에 두지 않는다.**
`01-Data-Pipeline/.secrets/` 아래에 두고 `.gitignore` 에 넣는다.
캘린더 MCP 의 `tokens.json` 은 건드리지 않는다 — 덮으면 달력 연결이 깨진다.

**④ 「알 수 없다」도 결론이다.**
데이터가 없거나 표본이 작으면 그렇게 적는다. 억지로 판정하지 않는다.
