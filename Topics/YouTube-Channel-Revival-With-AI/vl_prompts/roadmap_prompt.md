...
**Topic 설명**:
```
구독자 4,300명 부근에서 횡보하다 마이너스로 꺾였던 유튜브 채널을 AI와 함께 되살린
과정을, 감이 아니라 YouTube Analytics 데이터로 검증해 정리한다.
...
이어서 Substack 글을 쓴다. 영상은 발표를 녹화하므로 별도 제작하지 않는다.
```
...
**예상 학습 기간**: `6일 (2026-09-10 목 ~ 2026-09-16 수) · 총 17시간 · 발표일 고정`

---

### 환경 및 사전 지식

**운영 체제**: `Windows 11 Home (10.0.26200)`

**주요 도구 및 기술 스택**:
```
- Python 3.13
- google-api-python-client / google-auth-oauthlib  (YouTube API 인증·호출)
- YouTube Analytics API  (성과 지표 — views, estimatedMinutesWatched, subscribersGained/Lost 등)
- YouTube Data API v3    (영상 메타데이터 — 제목·공개일·길이·태그)
- YouTube Studio CSV export  (노출·CTR — Analytics API 에 없는 지표)
- pandas (선택 — 집계)
- Obsidian (문서·발표 자료)
- 기존 자산: Google Cloud 프로젝트 + OAuth 클라이언트가 **이미 있다**
  (`C:/Users/dougg/gcp-oauth.keys.json` — 캘린더 MCP 용으로 만들어 둔 것)
  같은 프로젝트에 YouTube API 두 개를 켜고 스코프만 추가하면 된다.
  ⚠️ 토큰 파일은 반드시 분리한다 — 캘린더 MCP 의 tokens.json 을 건드리면 달력 연결이 깨진다.
```
...
- 발표 슬라이드 작성 경험 (창발 2026-06-26 발표)
```
...
```
[이 Topic 의 1차 자료]
- vl_materials/2026-09-08 유튜브 채널 살리기 발표 브레인덤프 (원문).md
- vl_materials/2026-09-08 토픽 진행 계획.md   <- 모듈 구상·일정·리스크가 이미 정리돼 있다

[이어받는 선행 토픽 — 1편]
- Topics/The-AI-Powered-Creator/vl_materials/youtube-channel-growth-analysis.md  (P1~P4 Phase 분석)
- Topics/The-AI-Powered-Creator/vl_materials/youtube-analytics/  (CSV export 폴더 구조)
- Topics/The-AI-Powered-Creator/06-Slide-Deck/  (발표 덱 형식)

[볼트 안의 근거]
- AI/Tasks/Task Board.md 「유튜브 CSV 수동 분석」 — 가설 4개의 출처
- AI/Tasks/Task Board.md 「유튜브 채널 관리 애플리케이션」 — 지표 한계 정리
- Ingest/YouTube/_status — 채널 아카이브 496편, API 결과 검산 기준
- _Settings_/Skills/video-subtitles/ — 자막 작업을 스킬로 만든 증거물
...
**vl_materials/ 폴더**:
```
vl_materials/ 에는 이미 두 개가 들어 있다.
- 브레인덤프 원문 — **한 글자도 고치지 않은 사용자 원문.** 발표 문구는 여기서 나온다
- 토픽 진행 계획 — 모듈 구상(M1~M6), 일정, 리스크, 데이터 계획

M1 부터 여기에 추가될 것:
- youtube-analytics/  — YouTube Studio CSV export (1편과 같은 폴더 구조)
- raw JSON — Analytics API 응답 원본
```
...
사용자가 입력한 학습 기간 `6일 (2026-09-10 목 ~ 2026-09-16 수) · 총 17시간 · 발표일 고정`이 해당 Topic에 적절한지 분석하고 피드백을 제공하세요.
...
**사용자 입력 기간**: 6일 (2026-09-10 목 ~ 2026-09-16 수) · 총 17시간 · 발표일 고정
**Topic 복잡도**: [간단/중간/복잡]
**권장 기간**: [X주 또는 Y일]
