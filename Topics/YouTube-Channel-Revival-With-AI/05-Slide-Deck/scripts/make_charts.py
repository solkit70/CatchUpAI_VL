...
② 주별 순증 막대 — 작전 1(자막) · 작전 2(단일 주제, 7/16) · 전환점 8/24 주
③ 시청 지속률 — 15분 이하 영상만 · P4a 18.6% → P4b 초기 16.0% → 후기 27.2%

데이터: 01-Data-Pipeline/data/raw/daily.json (일별 gained/lost). 절대값은 2026-09-07 = 4,320
...
ANCHOR_DATE, ANCHOR_SUBS = date(2026, 9, 7), 4320     # 역산 기준 (M1)
FLAT_START, FLAT_END = date(2026, 3, 25), date(2026, 8, 23)   # P5a 횡보
FIRST_NEG_WEEK = date(2026, 3, 25)
OP1 = date(2026, 4, 3)      # 작전 1 (자막) 첫 라이브 파생 구간 시작 — operations.py 기준
OP2 = date(2026, 7, 16)     # 작전 2 단일 주제 첫 편
...
def chart_subscribers(rows):
    ...
    ax.annotate("전고점 돌파\n8/30 · 4,306", xy=(BREAK, dict(series)[BREAK]), xytext=(date(2026, 7, 20), 4345),
                fontsize=18, color=GREEN, arrowprops=dict(arrowstyle="->", color=GREEN, lw=2))
    ax.annotate(f"9/7 · {ANCHOR_SUBS:,}", xy=(ANCHOR_DATE, ANCHOR_SUBS), xytext=(date(2026, 8, 18), 4200),
                fontsize=16, color=FG, arrowprops=dict(arrowstyle="->", color=FG, lw=1.5))
    ax.set_ylim(4150, 4360)
    ax.set_title("구독자 수 — 2026년 2월 ~ 9월 7일", fontsize=24, color=FG, pad=16)
    ax.grid(axis="y", color=MUTE, alpha=0.25)
    for s in ("top", "right"): ax.spines[s].set_visible(False)
    fig.text(0.99, 0.01, "YouTube Analytics API · 일별 가입/해지 · 9/7 = 4,320 기준 역산", ha="right", fontsize=12, color=MUTE)
    fig.tight_layout()
...
def chart_weekly_net(rows):
    weeks = weekly_net(rows, date(2026, 3, 2), date(2026, 9, 7))
    ...
    for x, label, col in ((OP2, "작전 2 · 7/16\n단일 주제로 전환", AMBER), (TURN, "전환점 · 8/24 주", GREEN)):
        ax.axvline(x, color=col, ls="--", lw=2)
        ax.text(x + timedelta(days=1.5), max(ys) * 0.95, label, color=col, fontsize=17, va="top")
    ax.text(date(2026, 4, 1), max(ys) * 0.95, "작전 1 · 자막\n(세미나 34편)", color=BLUE, fontsize=17, va="top")
    ax.set_title("주별 순증 구독자 — 6주 0, 그다음 3주 급등", fontsize=24, color=FG, pad=16)
    ax.grid(axis="y", color=MUTE, alpha=0.25)
    for s in ("top", "right"): ax.spines[s].set_visible(False)
...
def main():
    ...
    print(f"횡보 구간 최저 {lo:,} · 최고 {hi:,} | 8/30 {series[BREAK]:,} | 8/24 {series[TURN]:,} | 9/7 {series[ANCHOR_DATE]:,}")
