...
이 프롬프트는 매일 학습을 시작할 때 사용하는 템플릿입니다.
...
**사용 절차**:
1. 아래 [1단계] 섹션을 현재 상황에 맞게 작성 (모듈·WorkLog·시간만 갱신)
2. 전체 내용을 AI에게 전달
...
**Topic 폴더 경로**:
```
C:\AI_study\2026\Changsoo_Vault\Ingest\CatchUpAI_VL\Topics\VibeLearn-AI-Chromebook\
```

**Roadmap 파일 경로**:
```
vl_roadmap/20260816_RoadMap_VibeLearn-AI-Chromebook.md
```
...
M5 - Chromebook판 VibeLearn AI 구축
M6 - 매뉴얼 + 온보딩 프로세스
M7 - 레포지토리 전략 확정 및 배포
M8 - Remotion 영상 제작
```
...
[다음 세션부터: vl_worklog/YYYYMMDD_MX_VibeLearn-AI-Chromebook.md]
```
...
[이 Topic에서 자주 쓰일 제약 예시]
- 실물 Chromebook 미확보 - 브라우저 기반 검증으로 대체
- 정책 원문 조사 위주 - 1차 출처 확인에 시간 배분 필요
- 영상 렌더링 세션 - 장시간 연속 작업 필요
```

---

### 이 Topic 고유 진행 원칙 (매 세션 적용)

```
1. M1·M2의 "실습"은 코딩이 아니라 1차 출처 확인 → 판정표·비교표 작성이다
2. 모든 정책 주장에 1차 출처 링크 + 조사 시점(2026-08) 명기
3. 2차 블로그만 근거인 항목은 "⚠️ 미확인" 표기
4. WorkLog에 확인한 1차 출처 URL과 조사 일자를 함께 남긴다
5. 실기기 확보 상황을 M2 종료 시점까지 매 세션 점검
6. 참조: vl_materials/00-Project-Plan.md (전체 계획 및 사전 리서치 근거)
```
...
**예시**: `vl_worklog/20260817_M1_VibeLearn-AI-Chromebook.md`
...
## 🔎 확인한 1차 출처 (이 Topic 고유 항목)

| 출처 | URL | 조사일 | 신뢰도 |
|------|-----|--------|--------|
| {문서명} | {URL} | 2026-08-XX | 1차 / ⚠️ 2차 |

> 정책·제도 정보는 빠르게 낡는다. 나중에 무엇을 다시 확인해야 하는지
> 추적하기 위해 조사한 출처와 날짜를 반드시 남긴다.
...
> **이 Topic 적용**: M1·M2에서 "실습"은 코딩이 아니라 1차 출처 확인 →
> 판정표·비교표 작성이다. 70-80% 원칙은 "직접 조사하고 표를 만든다"로 충족한다.
...
### 상황 5: 실물 Chromebook 미확보 (이 Topic 고유)

M4 시작 시점까지 실기기 또는 협조 교사가 확보되지 않은 경우:

```markdown
⚠️ **실기기 미확보 — 검증 방식 전환**

M4의 학생 트랙 실측을 다음으로 대체합니다:
- Chrome 브라우저 + 개인 Google 계정으로 기능 확인
- 관리형 환경 특유의 제약(정책 차단, 필터링)은 문서 기반 추론

**필수 조치**:
- 해당 산출물에 "⚠️ 관리형 환경 미검증" 표기
- M6 이후 실사용 피드백으로 보정할 항목 목록 작성
