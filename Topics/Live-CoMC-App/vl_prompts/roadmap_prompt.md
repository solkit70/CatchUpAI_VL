...
**Topic 설명**:
```
매주 일요일 3시간 라이브 방송(AI in Action Live)을 함께 진행할 로컬 데스크톱 보조
...
**예상 학습 기간**: `약 3개월 — 12주 (주당 7~8시간, 총 약 90시간)` *(2026-08-02 HITL-1 사용자 확정 — 최초 추정 6~8주/59시간에서 권장 범위 상한으로 확장)*

---

### 환경 및 사전 지식

**운영 체제**: `Windows 11 — i7-1355U (15W), 16GB RAM, GPU 없음 (클라우드 API 우선, 로컬 추론은 wake word/VAD 등 경량 작업만)`
...
- 파일 기반 파이프라인 패턴 선례: Topics/Vibe-Guiding-VSCode/04-Guiding-Engine-POC/
- API 연결 검증 하네스 패턴 선례: Topics/Qwen3-TTS/03-Setup-API/harness/connection_probe.py
- 학습 산출물과 실행 앱 분리 배치 선례: Topics/Remotion-VideoCreation/my-first-video/
- 방송 안전 규약 원본: _Settings_/Skills/live-broadcast/SKILL.md, _Settings_/Skills/rundown-writer/SKILL.md
- 승인된 상세 설계안(모듈 구성·아키텍처·안전장치·MVP 경계 전체): C:\Users\dougg\.claude\plans\ethereal-puzzling-seahorse.md (2026-08-02 사용자 승인) — 이 로드맵은 이 설계안을 VibeLearn AI 표준 형식으로 옮긴 것이며, 세부 근거는 원본 설계안을 참조한다
```

**vl_materials/ 폴더**:
```
- Live20/Live21 Weekly Rundown 실제 파일 사본 (파서 검증용) — 추가 예정
- 방송 안전 규약 사본 — 추가 예정
```
...
사용자가 입력한 학습 기간 `약 3개월 — 12주 (주당 7~8시간, 총 약 90시간)`이 해당 Topic에 적절한지 분석하고 피드백을 제공하세요. *(이 STEP 1은 2026-08-02에 이미 수행되어 사용자가 12주/90시간으로 확정했다 — 아래는 그 결과 기록이다)*
