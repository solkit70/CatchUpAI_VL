...
| 6 | LIVE 모드가 쓰는 `spoken.json` 을 **읽어 재생하는 코드가 없다** — 소리가 나갈 부품 자체가 부재 | 구조 | **M9** (Electron 셸 미완이 곧 그 자리) | 🔴 범위 밖으로 확정 (3회차 사전 점검 09-12) |
| 7 | `--serve` 중 파트 전환(핫키)이 컨텍스트를 다시 만들지 않음 — ④ 가 옛 파트 근거로 답함 | 구조 | **M9** (데몬 ①② 1회 실행) | 🟡 운영 우회 (파트마다 ② 재실행) |
