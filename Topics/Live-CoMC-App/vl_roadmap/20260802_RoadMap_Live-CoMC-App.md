...
| M5 | 2026-08-20 | 2026-08-20 | ✅ | 100% (7/8, 1건 N/A) | 호출어 전사 0/11 → STT 이전 감지 필수 확인. CER 16.7%→8.6%(표기 허용). gpt-live-transcribe 는 Realtime 전용이라 사다리 교정. 3사 어댑터 동일 스키마 수렴. BGM 조건 N/A(헤드폰 셋업). ✅ 이월 해소(8/23): 지연 원인은 추론 토큰. openai 15.2s→2.6s(minimal, 추론 1440→0tok) · gemini 11.1s→4.1s(low) · claude는 effort 무반응(7.0~7.7s). 품질 손실 없음. 폴백 순서를 openai→gemini→claude 로 바꿔야 함 |
| M6 | 2026-08-23 |  | 🔄 | 83% (5/6) | 실습 1·2·4 완료 + 대기 필러 설계(로드맵 외 추가). 기본값 edge-tts 확정(첫청크 588ms, 2위의 38%, 무료). **무료가 가장 빨라 로드맵의 '유료→무료 강등' 전제가 뒤집혔다** → 필요한 것은 가용성 승격. ElevenLabs 미검증(키 없음). 이월: 실습 3 라우팅 구성(미설치+재부팅 필요, 방송 중) · 주관 청취 평가 |
| M7 | 2026-08-23 | 2026-08-23 | ✅ | 94% (7/8, 1건 부분) | 실습 1~6 완료. 6단계 end-to-end 가동, claim_map 5샘플 위반 0건, 권위값/추정값 분리 테스트 통과. **지연 목표 미달(12.2s vs 2.5s) — 원인은 프로세스 기동 3.7s + 연결 설정 3.0s** → M9는 단일 장기 프로세스여야 한다. 근거 풀 크기가 LLM 지연 좌우(2→15건 +1.3s). 스키마 개정 4건(spoken.provider·part.is_numbered·intent.out_of_scope·suggested_part_id). 음성 지연은 M6 라우팅 후 측정 |
| M8 | 2026-08-30 | 2026-08-30 | ✅ | 100% (6/6) | 시나리오 6종(5 PASS + 1 관찰) + 결정적 테스트 9/9. **파서 결함 3종 발견 — 발화 항목 18개가 조용히 사라지고 있었다**(Live22 볼드 내 수식어 · Live25 괄호 없는 파트 헤딩 · '미편성'을 항목으로 오탐). 실패가 `undefined`라는 '안전해 보이는 값'으로 떨어져 아무도 눈치채지 못했다 → `parse_anomalies` 로 분리. **금칙 섹션 이름이 진행자 발화를 타고 프롬프트에 진입**해, 조건부 섹션 근거와 이어 붙여 없는 사실이 만들어졌다(게이트 통과) → 조건부 기본 제외 + `forbidden_topic_requested` 차단. 부재 주장을 화이트리스트 닫힘 근거로 재분류. 숫자·고유명사 사실검사(3종 검출, 오탐 0). LIVE/REVIEW/MUTE + 손상 시 REVIEW 폴백. 스키마 5종 확장. **볼트 Rundown 15편 전수 검증**에서 결함 2종 추가 발견 — `### 실험 ①②③` 하위 섹션 커버리지가 파트 커버리지를 **덮어쓰고** 있었다(Live16 3부·Live17 4부, 소실보다 나쁜 '대체') · Live11 이모지 접두 헤딩으로 파트 0개. 수정 후 15편 이상 0건. **effort 관문 통과** — 어려운 질문 6종×3회로 minimal 재검증, 게이트 통과 문장 중 근거 없는 것 0건이고 medium은 목표의 13배(33s)라 **minimal 유지 판정**(M5 이월분 해소) |
| M9 | 2026-08-31 | 2026-08-31 | 🔄 | 89% (8/9) | 상주 엔진으로 발화 9,084→4,369ms(52%↓). 핫키 15개·패닉 스톱 112ms·볼트 가드·OBS 오버레이 전부 실사용 검증 완료. 이월: 실습 1(Electron 셸 골격) — 급하지 않음, 핵심 기능은 전부 Python으로 동작. 오늘 발견: 권위값(session_state)과 스냅샷(overlay.json) 불일치로 화면이 핫키를 못 따라간 결함 1건(수정 완료) |
| M10 | 2026-09-06 | | 🔄 | 57% (4/7) | **Live #26 방송 중 진행.** 선결 과제였던 스트리밍 vs 안전 게이트는 **채택 안 함**으로 결론 — 필러가 1.4초 더 빨리 소리를 낸다(첫 소리 1.20s vs 2.6s). 부수로 M6 `T_filler` 가 오측정(케이블 314ms) 위에 있던 것을 **1.14초로 정정**. 프리플라이트 9항목 완성(첫 실행 FAIL 2건 실검출 → 수정 후 통과). 리허설 1회차에서 **사고 6건 발견·4건 수정**. ⭐ **최대 발견: 「내용 없는 발화」** — 게이트를 통과한 답이 커버리지 항목 이름을 하나도 말하지 않았다. 원인은 M8 게이트가 화이트리스트를 신뢰 출처로 인정하는데 M7 프롬프트는 재료에서 뺀 **두 층의 정의 불일치**. 프롬프트 수정만으로는 3회 중 1회만 성공해 **게이트 규칙 7 `content_empty` 신설**(대조 4조건 검증, 오탐 0). 회귀: 게이트 9/9 유지 · 볼트 16편 전수 파싱 이상 0건. **리허설 2회차 사고 0건** — 막혀야 할 경로 3종 전부 설계대로(미정 파트는 ②에서 차단·금칙 주제는 ③④에서 생성 거부·지연 중앙값 2,998ms로 M7 목표 2.5초 사정권, 단 회차가 달라 like-for-like 아님). 어미 붕괴는 10회 재측정 **0/10** 이라 프롬프트 수정으로 해소된 것으로 보이나, 하한을 위해 **게이트 규칙 8 `style_broken` 추가**(어미만 다른 두 문장으로 대조 검증, 낯선 어미는 안 막음). 판정: **REVIEW 투입 가능 · LIVE 조건부** — 1·2회차 모두 명령줄 `--text` 발화라 **마이크·호출어·STT·오디오 출력을 사람이 도는 전 구간은 미검증**(M4·M5·M6이 여기 걸려 있다). 3회차가 그 몫 |
