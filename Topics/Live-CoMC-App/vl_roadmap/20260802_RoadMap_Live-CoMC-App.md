...
- [x] Topic Retrospective 작성 (09-17 완결) 완료
- [ ] WorkLog 작성 완료
...
| M10 | 2026-09-06 | 2026-09-17 | ✅ | 100% (7/7) | **09-06 Live #26 방송 중 · 09-12 방송 전날.** 스트리밍 vs 안전 게이트 **채택 안 함**. 프리플라이트 9항목 · 리허설 1·2회차(사고 5→0) · 분류표 7건. **09-12: 3회차 = Live #27 실전으로 결정(사용자).** 사전 점검에서 구조 사고 2건 추가 — `spoken.json` 재생기 부재(LIVE 모드가 소리를 못 냄) · 파트 전환 시 컨텍스트 미갱신. **LIVE 판단 3갈래 문서화: A 화면 보조 조건부 GO · B 음성 출력 NO-GO · C 음성 입력 NO-GO.** **09-13 Live #27 = 3회차 실전: 시도 9 · 통과 6 · 침묵 3 · 사고 4건(8~11, 전부 화면에서 멈춤) · 코드 수정 0.** 09-17 결과 기록 + 진행자 소감 → Retrospective 완결. **A 판정은 무관중 4회차(09-18~19) 뒤 확정** (판정 규칙 「사고 있음 → 원인 기록 + 재리허설」 그대로). B·C 와 사고 8~11 수정은 이 Topic 의 **CVL 유지보수**로 |
...
전체 Topic 완료 기준:
- [ ] 모든 모듈 완료 (DoD 100%)
- [x] 최소 10개 산출물 폴더 생성 (M1~M10)
- [x] Topic Retrospective 작성 (09-17 완결)
- [ ] Self-Assessment 평균 ⭐⭐⭐⭐ 이상
- [x] Capstone(M10) — 무관중 2회 + 실전 1회. 최종 사고 0건은 아니나(3회차 4건) 11건 전부 유형·모듈·대책이 매핑됨 → 4회차(CVL)로
- [x] 실제 라이브 방송(AI in Action Live) 투입 여부에 대한 근거 있는 판단 완료 — 3갈래(A 보류→4회차 · B/C NO-GO)
