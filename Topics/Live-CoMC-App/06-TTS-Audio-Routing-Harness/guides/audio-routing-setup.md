...
| 5 | 재생 지연 실측 | ⚠️ **314ms → 정정 필요** (2026-08-31). 그 값은 **MME 경로**로 잰 것이다. 같은 케이블을 WASAPI 로 열면 `sd.play` 124ms · 저지연 스트림 63ms → [정정 근거](../../09-Desktop-Shell-and-Overlay/guides/panic-stop-benchmark.md#부수-발견--m6-의-케이블-314ms-는-케이블-때문이-아니었다) |
| 6 | OBS 프로필·씬 격리 | ✅ `CoMC-Test` 신설, 방송용 `Untitled` 무손상 |
| 7 | OBS 오디오 소스 2개 + 트랙 배정 | ✅ 진행자=트랙1, AI=트랙2 |
| 8 | OBS 모니터링 장치 | ✅ `Headphones (Shure MV7)` |
| 9 | **AI 음성 개별 차단 검증** | ✅ **완료 (2026-08-31)** — 아래 「킬 스위치 검증 완료」 |
| 10 | 방송용 트랙 배치 전환 | ⬜ **보류** — 실시간 차단은 소스 뮤트로 해결됐다. 트랙 분리는 후편집 필요 시에만 |
| 11 | 재측정 3종 | 🔄 **전제 재검토됨 (2026-08-31)** — 아래 「재측정 3종은 다시 따져야 한다」 |
...
## 2026-08-31 킬 스위치 검증 완료

**실시간 차단은 트랙 뮤트가 아니라 소스 뮤트다.** 이 문서가 앞서 세운 판단을 실측으로 닫았다.

### 무엇을 어디서 검증했나

⚠️ **`CoMC-Test` 가 아니라 방송용 `Untitled` 프로필에서 했다.** DoD 항목의 목적이
*"방송 중 AI 목소리만 끊고 진행자는 살린다"* 인데, 격리 프로필에서 확인해도
정작 방송하는 쪽은 미검증으로 남기 때문이다.

### 먼저 발견한 것 — 방송 프로필에 AI 음성이 아예 없었다

`Untitled` 의 오디오 믹서는 `Desktop Audio` + `Mic/Aux` 둘뿐이었다.
AI 음성은 `CABLE Input` 으로 나가는데, OBS `Desktop Audio` 는 **기본 재생 장치**
(당시 Galaxy Buds2 Pro)를 잡는다. **둘이 만나지 않는다.**

```
AI 음성 → CABLE Input ──╳── Desktop Audio (Galaxy Buds2 Pro)
```

→ `CABLE Output` 을 잡는 **오디오 입력 캡처 소스**를 추가해야 한다.
`CABLE Input` 이 *Default communications device* 로 잡혀 있어도 소용없다 —
OBS 는 communications 가 아니라 **playback 기본 장치**를 쓴다.

### 검증 절차

1. `Untitled` 씬에 오디오 입력 캡처 소스 추가 — 장치 `CABLE Output`
2. `CABLE Input` 으로 **1.5초 주기 880Hz 펄스**를 흘린다 (방에는 안 들림)
3. OBS 녹화를 켜고 **① 정상 → ② 소스 뮤트 → ③ 뮤트 해제** 순으로 조작하며 계속 말한다
4. 녹화본을 0.5초 창으로 FFT — **880Hz 대역**(AI)과 **150~700Hz**(사람)를 분리해 판정
...
|---|---|---|---|
| 0.0 ~ 5.0s | ████████████ | ████████ | 정상 |
| **5.0 ~ 11.0s** | **(없음)** | ████████████ | ✅ **AI만 차단, 진행자 생존** |
| 11.5 ~ 19.6s | ████████████ | █████████ | 복구 |

**880Hz 가 감쇠가 아니라 완전히 사라졌고**, 같은 구간에서 사람 목소리는 오히려
최대치를 찍었다. 뮤트 해제 후 즉시 복귀했다.

### 부수적으로 확인된 것

- **이중 포착 없음** — 톤 재생 시 `Desktop Audio` 는 무반응. 방송에 두 번 실리지 않는다
- **음향 에코 없음** — 톤 재생 시 `Mic/Aux` 무반응. 가상 케이블은 스피커를 거치지 않으므로
  마이크에 물리적으로 닿을 경로가 없다. → **에코 게이트 존치 판단에 직접 영향** (아래)

## 재측정 3종은 다시 따져야 한다

이 문서의 「4단계 재측정 명령」은 **VoiceMeeter 를 전제로 쓰였다.** 그런데 2026-08-23 에
VoiceMeeter 는 불필요로 판명돼 설치하지 않았다. 명령의 `--device "VoiceMeeter Out B1"` 은
**존재하지 않는 장치**이므로 그대로는 실행되지 않는다.

전제를 하나씩 다시 보면 이렇게 갈린다.

| # | 원래 재측정 사유 | 2026-08-31 판단 |
|---|---|---|
| ① M4 에코 게이트 | "신호 경로가 바뀌었다" | 🔄 **재판정 필요, 방향은 반대.** AI 음성이 스피커를 거치지 않아 **음향 에코 경로가 아예 없다.** 게이트가 불필요해질 수 있다 — 단 **헤드폰 모니터링 전제**이고, 스피커로 모니터링하면 다시 필요하다 |
| ② M5 WER | "VoiceMeeter 경유로 리샘플링이 한 번 더 들어간다" | ❌ **불필요.** VoiceMeeter 가 없으므로 마이크 경로가 M5 때와 동일하다. 재측정 사유가 소멸했다 |
| ③ M6 첫 청크 + 케이블 버퍼 | "파일 쓰기 기준으로만 쟀다" | ✅ **이미 완료.** 2026-08-23 에 `cable_ms 314` 실측 |

**결론 — 남은 것은 ① 하나이고, 그것도 "다시 재기"가 아니라 "존치 여부 판단"이다.**
현재 구성에서 에코가 구조적으로 불가능하다는 것은 오늘 확인했다. 남은 확인은
**진행자가 스피커로 모니터링하는 경우**뿐인데, 그 구성은 쓰지 않기로 이미 정해져 있다.

> 📌 위 「4단계 재측정 명령」 블록은 **역사 기록으로 남겨 둔다.** VoiceMeeter 전제로 쓰였다는
> 것을 알고 읽어야 하며, 그대로 실행하면 안 된다.

> 🔴 **2026-08-31 정정 — 이 314ms 는 케이블의 하한이 아니다.**
> `playback_latency.py` 가 장치를 `ci[0]`(첫 매칭)으로 고르는데, 이 환경에서 첫 매칭은
> **MME** 다. Windows 에서 지연이 가장 큰 호스트 API 다. 같은 케이블을 WASAPI 로 열면
> `sd.play` **124ms**, 저지연 스트림 **63ms** 다.
> *"줄일 수 없는 고정 비용"* 이라는 결론은 **철회한다.**
> → [정정 실측](../../09-Desktop-Shell-and-Overlay/guides/panic-stop-benchmark.md)

### 방송용 배치 (보류 — 2026-08-31)
