...
- [x] ✅ **실사용 검증 완료 (2026-08-29)** — 헤드폰 재생, OBS 화면 캡처, **마이크 음성과 데스크톱 오디오 모두 정상**. 라이브 방송·온라인 미팅 녹화에 지장 없음
- [ ] ⚪ **출력의 "기본 통신 장치"가 아직 `CABLE Input`** — 현재 사용에는 문제가 없다. 다만 **통신 장치를 따로 쓰는 앱**(Zoom·Teams 계열 일부 설정)에서는 상대 목소리가 안 들릴 수 있다. `mmsys.cpl` → 재생 탭에서 변경
- [ ] ⚪ OBS 오디오 장치를 "기본"이 아니라 **이름으로 고정** (`Speaker (Realtek(R) Audio)` / `Microphone (Shure MV7)`) — **VB-CABLE을 재설치·업데이트하면 같은 사고가 재발한다.** 지금 고쳐 두면 그때 안 흔들린다
