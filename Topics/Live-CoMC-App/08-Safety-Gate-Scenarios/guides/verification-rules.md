...
### 7-b · 9. Live #27 실전이 더한 검사 (CVL 1, 2026-09-17)

3회차 실전(09-13)의 사고 4건 중 셋은 **게이트를 통과한 채** 화면에 나갔다 — 근거는 있었고 인용도 실재했다. 틀린 것은 「무엇에 대한 답인가」였다.

| 규칙 | 사고 | 검사 | 판정 |
|---|---|---|---|
| **7-b 상태 질문** | 9 「어디까지 왔나」에 항목 이름만 | 질문에 `어디까지·상태·진행·현황·됐나·얼마나·끝났` 이 있으면, 남은 문장 중 하나는 상태 어휘(`완료·완수·진행 중·대기·보류·확정·미정·예정·남은·착수·마무리`)를 담아야 한다 | `content_empty` (규칙 7 과 같은 rule_id — 발화 보류, 화면은 유지) |
| **9 다른 파트** | 10 2부에서 「주간 영상」을 묻자 2부 항목으로 답 | 질문이 현재가 아닌 파트를 가리키면(`N부` 번호 불일치, 또는 다른 파트 제목 어휘 전부 포함) **전부** 드롭 | `cross_part` (새 탈락 사유) · rule_id `coverage.current_part_only` |

두 규칙의 판정 함수는 ④ 와 **같은 것**을 쓴다(`04_compose_answer.STATUS_KEYWORDS` · `other_part_mentioned`). ④ 는 LLM 을 부르기 전에 같은 판정으로 멈추고, ⑤ 는 ④ 를 믿지 않고 다시 잰다 — 정의가 두 벌이면 반드시 어긋난다(M8 회고).

**② 근거 풀도 바뀌었다** (사고 8). 확정 커버리지 항목이 `[확정] 이번 방송 커버리지 — …` 로 근거 풀 첫 줄에 들어가고, 커버리지 항목과 같은 표 행은 `[확정 항목 현재 상태] 이름 — 상태` 로 들어간다. `### 후보` 소절의 제목·불릿은 근거에서 뺀다. 이전에는 근거 풀에 「후보」라는 단어만 있고 확정 항목은 없어서, 모델이 시킨 대로 근거만 써서 확정을 후보라고 말했다(3/3 재현).

재현 테스트: `10-Live-Rehearsal-Capstone/examples/test_live27_incidents.py` — 수정 전 0/10 · 수정 후 10/10. M8 결정적 테스트 9/9 · 볼트 Rundown 18편 파싱 회귀 이상 0.
...
- [ ] **발화 길이가 문장 수로만 잘린다** — CVL 1 실측: 3문장이 소리로 36초. 방송에서 AI 가 36초를 말하면 진행이 멈춘다. 글자 수 상한(예: 220자)이 필요하다
- [ ] **`subsection_coverage` 를 실제로 쓰지는 않는다** — 담아만 뒀다. 진행자가 "실험 2 진행 중"이라고 하면 화이트리스트를 그쪽으로 좁히는 것이 M9 과제다

## CVL 4 (2026-09-17) — 두 번째 근거 풀: 캐주얼 레인

진행자 요청 「Rundown 만으로는 딱딱하다 — 날씨·이번 주 한 일·인사이트·잠깐 자리 비울 때 진행」. 원칙(근거 없으면 침묵)은 그대로 두고 **근거의 종류를 넓혔다.** 의도 6종(`small_talk` · `weekly_recap` · `insight` · `filler` · `broadcast_status` · `greet_viewer`)은 `broadcast_context` 가 아니라 `casual_brief.{live}.json`(②-b, `output/private/`)을 본다.

| 규칙 | 방송 레인 | 캐주얼 레인 | 왜 |
|---|---|---|---|
| 1·2 근거 실재 (claim_map · 인용 대조) | ✅ | ✅ | 틀린 말을 막는 층 — 그대로 |
| 3 화이트리스트 어휘 겹침 | ✅ | ✗ | 화이트리스트가 없다. 활용형 차이로 정상 문장이 떨어졌다 (실측) |
| 5 숫자·고유명사 | ✅ | ✅ | 날짜·기온·이름은 브리프 값 그대로여야 한다 |
| 7·7-b 내용 없음 / 상태 어휘 | ✅ | ✗ | '오늘 이 파트' 전제의 규칙 |
| 9 다른 파트 | ✅ | ✗ | 캐주얼 발화는 파트에 속하지 않는다. `broadcast_status` 는 오히려 파트를 넘어야 한다 |
| 8 어미 붕괴 | ✅ | ✅ | |
| 길이 | ~~120자~~ → **500자** · 7문장 (9/26 진행자 결정) | **500자** (`casual`) · 인사·날씨는 기본 500자 | 자리 비움을 채우려면 ≈50초 · 9/26: 120자로는 근거의 출처 문장이 잘려 올렸다 |
| `lang=en` | 규칙 3·7·라틴 고유명사 검사 생략, 상한 ×1.8 | 같음 | 영어 문장은 한국어 어휘로 잴 수 없다. 숫자 검사는 남는다 |

같이 잡은 것: 「오늘은 [확정] Chrome…」(사고 18, 라벨 누출) → ⑤ `strip_evidence_labels`. 「준비 마감은 어제 낮」(근거 9/19 를 상대 날짜로 변환한 환각 — 게이트가 못 잡는 종류) → 프롬프트에 오늘 날짜 + 「상대 표현 금지」. 「9/16」을 TTS 가 「16분의 9」로 읽음 → 재생기 `normalize_for_tts` (화면은 원문).
