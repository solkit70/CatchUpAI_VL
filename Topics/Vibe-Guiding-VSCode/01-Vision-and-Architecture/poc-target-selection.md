...
sources:
  ...
  - "[[Ingest/CatchUpAI_VL/Topics/GOBI-CLI/03-Space-Thread/README#M3 — Space Post & Reply 협업 기능|GOBI CLI Space Post/Reply]]"
...
|---|---|---|---|---|---|---|
| GOBI CLI | 높음. `gobi --version`, `gobi auth status`, `gobi space list` 같은 명령으로 재현 가능 | 높음. 설치 여부, 인증 상태, Space/Post 접근을 명령 출력으로 수집 가능 | 낮음. 명령 성공/실패와 JSON 출력으로 검증 가능 | 높음. `GOBI-CLI` Topic에 Setup/Auth, Space Post/Reply, Capstone 자료가 이미 있음 | 중간-높음. 기술 데모로 명확하고 이후 제품 통합의 기반이 된다 | **1차 POC 대상으로 선택** |
...
GOBI CLI는 첫 POC에서 필요한 네 가지 파일 흐름을 가장 선명하게 검증할 수 있다. `user_context.json`에는 OS, CLI 설치 여부, CLI 버전, 인증 상태, 선택된 Space, 명령 실행 가능 여부를 넣을 수 있다. GOBI CLI v2.0.12 기준으로 `trigger_rules.json`에는 "gobi 명령을 찾을 수 없음", "인증 상태가 아님", "Space 선택이 안 됨", "Post 생성 실패" 같은 problem signal을 정의할 수 있다.

이미 [[Ingest/CatchUpAI_VL/Topics/GOBI-CLI/01-Setup-Auth/README#M1 — 설치 & 인증 & 핵심 개념|GOBI CLI Setup/Auth]]와 [[Ingest/CatchUpAI_VL/Topics/GOBI-CLI/03-Space-Thread/README#M3 — Space Post & Reply 협업 기능|Space Post/Reply 자료]]가 있으므로 Retrieval Index 초안을 만들기 쉽다. 또한 [[Ingest/CatchUpAI_VL/Topics/GOBI-CLI/04-Capstone/README#M4 — 실전 워크플로우 + Quick Reference|Capstone workflow]]가 있어, POC의 end-to-end 테스트 시나리오를 `auth check → Space 확인 → Post 생성 안내` 흐름으로 잡을 수 있다.
...
| Space 작업 막힘 | Space 목록 또는 Post 생성 실패 | selectedSpaceSlug, `gobi space list`, `gobi space list-posts` 결과 | Space 선택, Post 생성 명령, 실패 시 fallback |
