...
## GobiSpace 별도 시스템 프롬프트 지원 (2026-07-12 정정 — 단일 필드로 확인됨)

> ⚠️ **정정 (2026-07-12)**: 아래는 M1 시점(6/28) 추정이었다. 2026-07-12 실제 어드민 화면(Settings → Agents) 확인 결과 **System prompt 입력 필드는 하나뿐**이며, mention/chat 별도 필드는 존재하지 않는다. Language도 "Korean 고정"이 아니라 "Auto"로 표시됨. 아래 표는 오답이므로 참고용으로만 남기고, 실제 적용은 통합 프롬프트(v2.1, WorkLog 세션2 참조)를 사용한다.

강민석님이 제공한 두 파일의 Agent Prompt 섹션이 다른 이유로 추정했던 내용(M1 시점, 오답):

| 컨텍스트           | 시스템 프롬프트   | 강민석님 테스트 Agent Prompt                    |
| -------------- | ---------- | ---------------------------------------- |
| @mention (포스트) | 별도 프롬프트 적용(추정) | `항상 농담과 함께 답변해줘`                         |
| 채팅 (말풍선)       | 별도 프롬프트 적용(추정) | `항상 농담과 함께 답변해줘. 각 문장마다 다람쥐 라는 말로 끝내 줘.` |

→ 실제로는 단일 System prompt 필드 하나로 mention·chat 양쪽에 모두 적용되는 것으로 보인다.
