...
**학습 목적**:
```
- 두 기능을 "써 보니 되더라" 수준에서 "무엇이 되고 무엇이 안 되는지 알고 쓰는" 수준으로 올린다 — 특히 「db 연동 페이지는 왜 링크 공유가 안 되는가」를 정확히 답한다
...
- HTML/CSS/JS 를 읽고 고칠 수 있는 정도
- 이미 만든 실물 4건의 맥락 — Materials_For_Topics/Claude-Artifacts-Routines/2026-09-13 사례 인덱스.md

권장:
- cron 표현식
- VibeLearn AI 영상 제작 흐름 (remotion-video 스킬)
- Personal-Ops-Board PRD (Deadline 에이전트를 루틴으로 돌릴 수 있는지가 실습 후보)
```
...
**참조 자료**:
```
- Claude Code Artifacts 공식 문서 (docs.claude.com) — M2 에서 정확한 페이지를 찾아 클리핑 (현재 볼트에 클리핑 0건)
- Claude Code Routines / Scheduled cloud agents 공식 문서 — M4 에서 클리핑
- 세션 내 스킬: artifact-capabilities · artifact-design · schedule (Skill 도구로 로드)
...
**vl_materials/ 폴더**:
```
- 시작 시점: 비어 있음. Materials_For_Topics/Claude-Artifacts-Routines/ 의 두 문서가 출발 자료
- M2·M4 에서 공식 문서 클리핑과 화면 스크린샷을 추가
- 실물 링크 4건: 부스 매니저 991fb41f-5007-4f15-9c3b-429ab0e33b2a · 부스 현황판 aa2d46bf-d01c-4864-96c2-9257c7bd3e0e · 입장 안내 e9c029bd-e072-45a9-9618-a47881af3676 · WBLP 루틴 trig_01LKgvHWt4ZfJbTqJgn5KvVT
```
...
사용자가 입력한 학습 기간 `3주 (주당 약 5시간, 총 15시간)`이 해당 Topic에 적절한지 분석하고 피드백을 제공하세요.
