...
**Topic 폴더 경로**:
```
C:/AI_study/2026/Changsoo_Vault/Ingest/CatchUpAI_VL/Topics/Seattle-AI-Week-2026/
```
...
**현재 진행 중인 모듈**:
```
M1 - 행사 전수 수집과 사실표 (기한 9/20) → M2 큐레이션·티켓 (기한 9/27)
```

**가장 최근 WorkLog 파일**:
```
[없음 - 첫 학습 세션] (세션마다 최신 WorkLog 로 갱신)
```

**이전 세션의 "Tomorrow's focus"** (있다면):
```
[없음] — 이후 직전 WorkLog 의 Tomorrow's focus 와 「개선할 점」을 옮겨 적는다
```
...
**사용 가능한 시간**:
```
[세션마다 입력 — 사전 1~2h · 현장 주간(10/26~30)은 하루 단위]
```

**오늘의 상태/제약사항** (있다면):
```
[세션마다 입력] — 기한 우선: M2 티켓 9/27 · 9/16 BL5 발표 · 9/26 BigHug · 9/27 Personal Ops Board 착수와 겹침
```
