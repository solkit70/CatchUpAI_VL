...
원격 호스트에서 브라우저나 데스크톱 작업을 시키는 기능이다.

> ✅ **2026-08-30 브라우저 부분 실측 완료** → [../lab/browser-control-verification.md](../lab/browser-control-verification.md)

**브라우저는 데스크톱 앱에 내장되어 있다.** 이전에 여기 *"Chrome 확장 설치가 필요하다"* 라고 적었던 것은 **정확하지 않았다.** 둘은 다른 물건이다.

| | 내장 브라우저 | Codex Chrome 확장 |
|---|---|---|
| 설치 | 앱에 이미 있음 (`Ctrl+Shift+B` / `⌘+Shift+B`) | 별도 설치 |
| 브라우저 상태 | 앱 전용 **독립 상태** | 쓰던 Chrome 그대로 |
| 기존 로그인·확장·탭 | 없음 | 있음 |

확장은 **"쓰던 Chrome 세션이 필요할 때"** 쓰는 것이지 전제 조건이 아니다.

**Windows에서는 세 가지 제약이 있다.**

- 세션이 **잠금 해제 상태**여야 한다
- **포그라운드에서 실행**된다 — 작업 중 다른 일을 하기 어렵다
- **모바일에서는 브라우저 화면을 볼 수 없다** ← 실측으로 확인

### 화면을 못 본다는 것의 무게

원격에서 지시하면 **호스트 컴퓨터에서는** 실제로 페이지가 열린다. 그런데 **모바일 앱에는 그 화면이 뜨지 않고** 텍스트 응답만 온다.

앞의 두 제약은 준비 조건이지만 이건 다르다. **밖에서 쓰려고 켜는 기능인데 밖에서는 결과를 확인할 수 없다.** 목적 자체가 무너진다.

### 로그인 세션 이전은 자격증명 이전이다

내장 브라우저에 기존 로그인을 쓰려면 Chrome에서 **저장된 비밀번호 · 쿠키 · 방문 기록**을 가져와야 하고, Windows App-Bound Encryption 때문에 **관리자 승인**까지 필요하다. 편의 기능으로 보고 누를 일이 아니다.
...
| **모바일에서 브라우저 화면 보기** | **불가.** 조작은 되지만 화면은 호스트에만 뜬다 (실측) |
