...
5. 생성된 로드맵을 `vl_roadmap/YYYYMMDD_RoadMap_Slack-Builders-Lounge-Automation.md`에 저장
...
**Topic 설명**:
```
changbal.slack.com의 #club-sg-ai 채널에서 Builders Lounge 관련 Slack 글을 가져와 AI/Initiatives/Builders Lounge/slack/ 아래 Markdown 문서로 자동 정리하는 방법을 연구하고 구현합니다.
```

**학습 목적**:
```
- Slack Web API의 conversations.history, conversations.replies, users, permalink, OAuth scope 구조를 AI에게 지시할 수 있을 정도로 이해합니다.
- 현재 수동으로 진행하는 Slack 글 수집과 Markdown 정리 작업을 자동화합니다.
- 기존 Builders Lounge Slack 문서와 호환되는 Markdown schema와 증분 sync 구조를 설계합니다.
- GitHub 공개 공유 전 privacy/review workflow를 포함합니다.
```

**예상 학습 기간**: `1주 또는 8-10시간 집중 연구/개발`

---

### 환경 및 사전 지식

**운영 체제**: `Windows 11 / Obsidian Vault / PowerShell`
...
**학습 목표** (달성하고 싶은 것):
```
- Slack API 접근 방식, 필요한 권한, rate limit, workspace export 대안을 정리합니다.
- #club-sg-ai 채널의 메시지, 스레드, 작성자, permalink를 가져오는 prototype을 만듭니다.
- 기존 AI/Initiatives/Builders Lounge/slack/ 문서와 호환되는 Markdown schema를 정의합니다.
- 마지막 수집 시각을 저장하는 증분 동기화 구조를 만듭니다.
- 공개 GitHub 공유 전 검토와 redaction 기준을 자동화 흐름에 포함합니다.
- 실제 로컬 실행 가능한 Slack-to-Markdown sync 스크립트를 만듭니다.
```
...
- Existing Slack notes: AI/Initiatives/Builders Lounge/slack/
```
...
사용자가 입력한 학습 기간 `1주 또는 8-10시간 집중 연구/개발`이 해당 Topic에 적절한지 분석하고 피드백을 제공하세요.
...
**명명 규칙**:
- 모듈: `M1`, `M2`, `M3`, ...
- 산출물 폴더: `01-Slack-Builders-Lounge-Automation/`, `02-Slack-Builders-Lounge-Automation/`, ...
...
**파일명 규칙**: `vl_worklog/YYYYMMDD_MX_Slack-Builders-Lounge-Automation.md`
...
`vl_worklog/YYYYMMDD_Slack-Builders-Lounge-Automation_Final_Retrospective.md`:
- 전체 학습 여정 통계
...
├── vl_roadmap/
│   └── YYYYMMDD_RoadMap_Slack-Builders-Lounge-Automation.md  # 생성될 로드맵
├── vl_worklog/
│   ├── YYYYMMDD_M1_Slack-Builders-Lounge-Automation.md
│   ├── YYYYMMDD_M2_Slack-Builders-Lounge-Automation.md
│   └── ...
...
다음 Markdown 형식으로 로드맵을 생성하고 `vl_roadmap/YYYYMMDD_RoadMap_Slack-Builders-Lounge-Automation.md`에 저장하세요.

### 로드맵 템플릿 구조

```markdown
# Slack-Builders-Lounge-Automation 학습 로드맵
...
### 학습 환경
- OS: Windows 11 / Obsidian Vault / PowerShell
- 도구: {기술 스택}
- 사전 지식: Slack workspace access, API token, Python, Markdown vault structure
...
1. [ ] `vl_roadmap/YYYYMMDD_RoadMap_Slack-Builders-Lounge-Automation.md` 파일로 저장
