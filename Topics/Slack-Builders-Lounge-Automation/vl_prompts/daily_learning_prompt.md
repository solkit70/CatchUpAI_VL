...
이 프롬프트는 Slack-Builders-Lounge-Automation Topic의 daily learning 세션을 시작할 때 사용합니다.
...
**Topic 폴더 경로**:
```
C:\AI_study\2026\Changsoo_Vault\Ingest\CatchUpAI_VL\Topics\Slack-Builders-Lounge-Automation\
```

**Roadmap 파일 경로**:
```
vl_roadmap/20260705_RoadMap_Slack-Builders-Lounge-Automation.md
```
...
**오늘의 상태/제약사항** (있다면):
```
실제 Slack token은 아직 준비되지 않았을 수 있으므로, 첫 세션은 Slack API 권한/접근 방식 확정과 fixture 기반 설계까지 진행한다.
```
...
**WorkLog**:
- `vl_worklog/YYYYMMDD_M1_Slack-Builders-Lounge-Automation.md`

**산출물 폴더**:
- `01-Slack 자동화 범위와 API 접근 방식 확정/`
  - README.md
  - concepts/{파일명}.md
  - examples/{파일명}.py
  - (기타)
...
**파일 경로**: `vl_worklog/YYYYMMDD_M1_Slack-Builders-Lounge-Automation.md`
