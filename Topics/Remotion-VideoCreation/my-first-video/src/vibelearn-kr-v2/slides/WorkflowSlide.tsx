    ...
    <AbsoluteFill style={{ opacity: fadeOut }}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 0, flex: 1 }}>
          {items.map((item, i) => {
            const DELAY = 15 + i * 20;
            const stepSpring = spring({ frame: frame - DELAY, fps, config: { damping: 14, stiffness: 80 } });
            const stepOpacity = interpolate(frame - DELAY, [0, 20], [0, 1], { extrapolateRight: 'clamp' });
            ...
            return (
              <div
                key={i}
                style={{ display: 'flex', alignItems: 'center', flex: 1, opacity: stepOpacity }}
              >
                <div
                  style={{
                    flex: 1,
                ...
                >
                  ...
                  <div style={{ fontSize: 18, fontWeight: 700, color, fontFamily: 'sans-serif', textAlign: 'center' }}>
                    {item.text}
                  </div>
                  {item.sub && (
                    <div style={{ fontSize: 14, color: COLORS.textMuted, fontFamily: 'sans-serif', textAlign: 'center', lineHeight: 1.4 }}>
                      {item.sub}
                    </div>
                  )}
                </div>
                ...
                {!isLast && (
                  <div
                    style={{
                      ...
                      fontSize: 28,
                      padding: '0 8px',
                      opacity: interpolate(frame - DELAY - 10, [0, 15], [0, 1], { extrapolateRight: 'clamp' }),
                    }}
                  >
                    →
                  </div>
