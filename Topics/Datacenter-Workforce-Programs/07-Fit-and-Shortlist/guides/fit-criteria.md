...
| **예외 (2026-08-31 신설)** | **단기 전일 집중 체류**는 아래 4조건을 모두 만족하면 허용 |
| 거리 상한 | 거주지(Bonney Lake) 기준 **편도 200마일** (통학형에 한함) |
| 근거 | 사용자 응답 (2026-08-30) · **개정 (2026-08-31)** |

**BBCC(Moses Lake) 편도 180마일은 이 기준을 통과한다.** 다만 왕복 360마일·약 5~6시간이므로 **주 3일 이상이면 탈락**이다. → M6 문의 질문 1·3번의 답이 이 판정을 확정한다.

#### ⚠️ 2026-08-31 개정 — 기준이 상정하지 않은 형태가 나타났다

M8 에서 **① Meta AWA** 를 확인했다. **타주에서 4주 연속 전일 체류**를 요구한다.
탈락선을 문자 그대로 적용하면 탈락이다. **그런데 이 탈락선은 이 형태를 상정하고 만든 것이 아니다.**

기준을 세울 때(8/30) 눈앞에 있던 후보는 전부 **통학형**이었다 — BBCC 통학, JATC 야간 수업.
그래서 탈락선의 단위가 **「주당 며칠」** 이 됐다. **장기간 반복되는 왕복 부담**을 재는 자였다.

| | 이 기준이 재려던 것 | Meta AWA |
|---|---|---|
| 지속 | **1년간 반복** | **4주, 한 번** |
| 왕복 | 매주 발생 | **왕복 2회로 끝** |
| 비용 | 본인 부담 | **여비·숙박 프로그램 부담** |
| 수입 | 없음 | **훈련 중 급여 지급** |
| 끝난 뒤 | 수료증 (취업은 별개) | **합격 시점에 조건부 채용 제안** |

> **「주 3일」이라는 자로 「4주 한 번」을 재면 눈금이 맞지 않는다.**
> 주당 일수는 **반복 부담**을 재는 단위인데, AWA 의 부담은 반복이 아니라 **1회성 이동**이다.

#### 사용자 결정 (2026-08-31)

> **타주 4주 체류를 수용합니다.**

#### 예외 조건 — 무엇이든 통과시키지 않기 위해

기준을 한 번 구부리면 다음에도 구부러진다. **예외의 범위를 명시한다.**
아래 **4개를 모두** 만족할 때만 단기 전일 체류를 허용한다.

| # | 조건 | AWA |
|---|---|---|
| 1 | **연속 4주 이내**이고 **반복되지 않는다** | ✅ 4주 1회 |
| 2 | **여비·숙박을 프로그램이 부담**한다 | ✅ 전액 |
| 3 | **훈련 중 수입이 있다** (기준 4와 연동) | ✅ 급여 지급 |
| 4 | **수료 후 고용이 보장 또는 조건부 확약**된다 | ✅ 합격 시 조건부 채용 제안 |

**하나라도 빠지면 원래 탈락선을 적용한다.** 특히 2·3번이 빠진 4주 타주 체류는
**무수입 상태로 4주치 숙박비를 내는 것**이므로 성격이 완전히 다르다.

> **배운 것 — 기준을 만들 때 눈앞에 있던 후보의 형태가 기준의 단위를 결정한다.**
> 통학형만 보고 있으면 자가 「주당 일수」가 되고, 그 자로는 **집중형을 잴 수 없다.**
