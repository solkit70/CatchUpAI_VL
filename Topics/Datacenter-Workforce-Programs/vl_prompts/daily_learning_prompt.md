# VibeLearn AI Daily Learning 프롬프트 — Datacenter-Workforce-Programs
...
**Topic 폴더 경로**:
```
Ingest/CatchUpAI_VL/Topics/Datacenter-Workforce-Programs/
```

**Roadmap 파일 경로**:
```
vl_roadmap/20260823_RoadMap_Datacenter-Workforce-Programs.md
```
...
### ⚠️ 이 Topic 세션마다 확인할 것

1. **모집 마감이 임박한 건이 있는가** — M2 `intake-calendar.md` 를 매 세션 시작 시 확인한다.
   4주 이내 마감 건이 생기면 정상 모듈 순서보다 그 건의 지원 준비를 먼저 한다.
2. **확인 불가 항목이 쌓여 있는가** — 이전 WorkLog의 "확인 불가"를 이번 세션에 해소할 수 있는지 본다.
   기관 문의는 영업시간이 있으므로 세션 앞부분에 배치한다.
3. **M7 이전이면 트랙을 기울이지 않았는가** — 숙련직·기술직 중 한쪽 정보만 늘고 있지 않은지 점검한다.
4. **개인정보가 볼트에 저장되지 않았는가** — M8 진행 시 특히 확인한다.
