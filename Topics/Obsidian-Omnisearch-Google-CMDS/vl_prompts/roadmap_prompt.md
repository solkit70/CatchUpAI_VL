...
5. 생성된 로드맵을 `vl_roadmap/YYYYMMDD_RoadMap_Obsidian-Omnisearch-Google-CMDS.md`에 저장
...
**Topic 설명**:
```
Google 검색 화면에서 Obsidian Omnisearch 결과를 함께 표시하는 Tampermonkey userscript를 학습하고 현재 컴퓨터에 설치·검증한다.
```
...
- Local source map: vl_materials/source-map.md
```

**vl_materials/ 폴더**:
```
vl_materials/source-map.md에 repo, userscript URL, 설치 대상 vault, 의존 플러그인 버전, 방송 슬라이드 요약을 기록한다.
```
...
사용자가 입력한 학습 기간 `1-2일, 총 3-5시간`이 해당 Topic에 적절한지 분석하고 피드백을 제공하세요.
...
**명명 규칙**:
- 모듈: `M1`, `M2`, `M3`, ...
- 산출물 폴더: `01-Obsidian-Omnisearch-Google-CMDS/`, `02-Obsidian-Omnisearch-Google-CMDS/`, ...
...
**파일명 규칙**: `vl_worklog/YYYYMMDD_MX_Obsidian-Omnisearch-Google-CMDS.md`
...
`vl_worklog/YYYYMMDD_Obsidian-Omnisearch-Google-CMDS_Final_Retrospective.md`:
- 전체 학습 여정 통계
...
├── vl_roadmap/
│   └── YYYYMMDD_RoadMap_Obsidian-Omnisearch-Google-CMDS.md  # 생성될 로드맵
├── vl_worklog/
│   ├── YYYYMMDD_M1_Obsidian-Omnisearch-Google-CMDS.md
│   ├── YYYYMMDD_M2_Obsidian-Omnisearch-Google-CMDS.md
│   └── ...
...
다음 Markdown 형식으로 로드맵을 생성하고 `vl_roadmap/YYYYMMDD_RoadMap_Obsidian-Omnisearch-Google-CMDS.md`에 저장하세요.
...
### 학습 환경
- OS: Windows 11
- 도구: Obsidian Desktop, Chrome 또는 Edge, Tampermonkey, Omnisearch, Local REST API
- 사전 지식: Obsidian community plugin, browser extension permission, localhost HTTP server concepts
...
1. [ ] `vl_roadmap/YYYYMMDD_RoadMap_Obsidian-Omnisearch-Google-CMDS.md` 파일로 저장
